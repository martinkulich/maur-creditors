...
$$eventGroup = new sfEvent($task, 'task.test.filter_test_files', array('arguments' => array(), 'options' => array()));
$files = $pluginConfig->filterTestFiles($$eventGroup, array());
...
$$eventGroup = new sfEvent($task, 'task.test.filter_test_files', array('arguments' => array('controller' => array()), 'options' => array()));
$files = $pluginConfig->filterTestFiles($$eventGroup, array());
$t->is(count($files), 3, '->filterTestFiles() adds functional plugin tests');

$task = new sfTestFunctionalTask($configuration->getEventDispatcher(), new sfFormatter());
$$eventGroup = new sfEvent($task, 'task.test.filter_test_files', array('arguments' => array('controller' => array('BarFunctional')), 'options' => array()));
$files = $pluginConfig->filterTestFiles($$eventGroup, array());
$t->is(count($files), 1, '->filterTestFiles() adds functional plugin tests when a controller is specified');

$task = new sfTestFunctionalTask($configuration->getEventDispatcher(), new sfFormatter());
$$eventGroup = new sfEvent($task, 'task.test.filter_test_files', array('arguments' => array('controller' => array('nested/NestedFunctional')), 'options' => array()));
$files = $pluginConfig->filterTestFiles($$eventGroup, array());
...
$$eventGroup = new sfEvent($task, 'task.test.filter_test_files', array('arguments' => array('name' => array()), 'options' => array()));
$files = $pluginConfig->filterTestFiles($$eventGroup, array());
$t->is(count($files), 3, '->filterTestFiles() adds unit plugin tests');

$task = new sfTestUnitTask($configuration->getEventDispatcher(), new sfFormatter());
$$eventGroup = new sfEvent($task, 'task.test.filter_test_files', array('arguments' => array('name' => array('FooUnit')), 'options' => array()));
$files = $pluginConfig->filterTestFiles($$eventGroup, array());
$t->is(count($files), 1, '->filterTestFiles() adds unit plugin tests when a name is specified');

$task = new sfTestUnitTask($configuration->getEventDispatcher(), new sfFormatter());
$$eventGroup = new sfEvent($task, 'task.test.filter_test_files', array('arguments' => array('name' => array('nested/NestedUnit')), 'options' => array()));
$files = $pluginConfig->filterTestFiles($$eventGroup, array());
