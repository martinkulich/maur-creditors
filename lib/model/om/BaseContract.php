...
abstract class BaseContract extends BaseObject  implements Persistent {
	 ...
	 * actually need in Contract.
	 */
	public function getPaymentsJoinBankAccount($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(ContractPeer::DATABASE_NAME);
		}
		...
		if ($this->collPayments === null) {
			if ($this->isNew()) {
				...
				$this->collPayments = PaymentPeer::doSelectJoinBankAccount($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
