...
abstract class BaseContract extends BaseObject  implements Persistent {
	 ...
	 */
	public function getSettlementsJoinOutgoingPayment($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(ContractPeer::DATABASE_NAME);
		}
		...
		if ($this->collSettlements === null) {
			if ($this->isNew()) {
				...
				$this->collSettlements = SettlementPeer::doSelectJoinOutgoingPayment($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
