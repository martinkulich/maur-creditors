...
/**
 * Base class that represents a row from the 'gift' table.
 *
 * 
 *
 * @package    lib.model.om
 */
abstract class BaseGift extends BaseObject  implements Persistent {
	...
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = GiftPeer::ID;
		}
	...
	public function setName($v)
	{
		...
		if ($this->name !== $v) {
			$this->name = $v;
			$this->modifiedColumns[] = GiftPeer::NAME;
		}
	...
	public function setDate($v)
	{
		...
		if ( $this->date !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = GiftPeer::DATE;
			}
	...
	public function setNote($v)
	{
		...
		if ($this->note !== $v) {
			$this->note = $v;
			$this->modifiedColumns[] = GiftPeer::NOTE;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 5; // 5 = GiftPeer::NUM_COLUMNS - GiftPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating Gift object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = GiftPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseGift:delete:pre') as $callable)
			{
			...
			if ($ret) {
				GiftPeer::doDelete($this, $con);
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseGift:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseGift:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseGift:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				GiftPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					$affectedRows += GiftPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = GiftPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = GiftPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = GiftPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(GiftPeer::ID)) $criteria->add(GiftPeer::ID, $this->id);
		if ($this->isColumnModified(GiftPeer::CREDITOR_ID)) $criteria->add(GiftPeer::CREDITOR_ID, $this->creditor_id);
		if ($this->isColumnModified(GiftPeer::NAME)) $criteria->add(GiftPeer::NAME, $this->name);
		if ($this->isColumnModified(GiftPeer::DATE)) $criteria->add(GiftPeer::DATE, $this->date);
		if ($this->isColumnModified(GiftPeer::NOTE)) $criteria->add(GiftPeer::NOTE, $this->note);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(GiftPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Gift (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Gift Clone of current object.
	 * @throws     PropelException
	 */
	...
	{
	  if (!$callable = sfMixer::getCallable('BaseGift:'.$method))
	  {
	    throw new sfException(sprintf('Call to undefined method BaseGift::%s', $method));
	  }
