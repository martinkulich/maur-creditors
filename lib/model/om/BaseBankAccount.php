...
abstract class BaseBankAccount extends BaseObject  implements Persistent {
	...
	public function setName($v)
	{
		...
		if ($this->name !== $v) {
			$this->name = $v;
			$this->modifiedColumns[] = BankAccountPeer::NAME;
		}
	...
	public function setNumber($v)
	{
		...
		if ($this->number !== $v) {
			$this->number = $v;
			$this->modifiedColumns[] = BankAccountPeer::NUMBER;
		}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(BankAccountPeer::NAME)) $criteria->add(BankAccountPeer::NAME, $this->name);
		if ($this->isColumnModified(BankAccountPeer::NUMBER)) $criteria->add(BankAccountPeer::NUMBER, $this->number);
	 ...
	 * related OutgoingPayments from storage. If this BankAccount is new, it will return
	...
	public function getOutgoingPaymentsJoinCurrency($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		...
		if ($this->collOutgoingPayments === null) {
			if ($this->isNew()) {
				...
				$this->collOutgoingPayments = OutgoingPaymentPeer::doSelectJoinCurrency($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
