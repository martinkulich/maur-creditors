...
/**
 * Base class that represents a row from the 'regulation_year' table.
 *
 * 
 *
 * @package    lib.model.om
 */
abstract class BaseRegulationYear extends BaseObject  implements Persistent {
	 ...
	 * @return     RegulationYear The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = RegulationYearPeer::ID;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 1; // 1 = RegulationYearPeer::NUM_COLUMNS - RegulationYearPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating RegulationYear object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = RegulationYearPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseRegulationYear:delete:pre') as $callable)
			{
			...
			if ($ret) {
				RegulationYearPeer::doDelete($this, $con);
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseRegulationYear:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseRegulationYear:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseRegulationYear:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				RegulationYearPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = RegulationYearPeer::doInsert($this, $con);
					...
					$affectedRows += RegulationYearPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = RegulationYearPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = RegulationYearPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = RegulationYearPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
		);
		return $result;
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(RegulationYearPeer::ID)) $criteria->add(RegulationYearPeer::ID, $this->id);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(RegulationYearPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of RegulationYear (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     RegulationYear Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     RegulationYearPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new RegulationYearPeer();
		}
		return self::$peer;
	}
	 ...
	 * Otherwise if this RegulationYear has previously been saved, it will retrieve
	 * related Regulations from storage. If this RegulationYear is new, it will return
	...
	public function addRegulation(Regulation $l)
	{
		...
		if (!in_array($l, $this->collRegulations, true)) { // only add it if the **same** object is not already associated
			...
			$l->setRegulationYearRelatedByRegulationYear($this);
		}
	}
	...
	{
	  if (!$callable = sfMixer::getCallable('BaseRegulationYear:'.$method))
	  {
	    throw new sfException(sprintf('Call to undefined method BaseRegulationYear::%s', $method));
	  }
