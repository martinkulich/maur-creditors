...
abstract class BaseReport extends BaseObject  implements Persistent {
	...
	public function setCode($v)
	{
		...
		if ($this->code !== $v) {
			$this->code = $v;
			$this->modifiedColumns[] = ReportPeer::CODE;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 2; // 2 = ReportPeer::NUM_COLUMNS - ReportPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(ReportPeer::CODE)) $criteria->add(ReportPeer::CODE, $this->code);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(ReportPeer::CODE, $this->code);
	 ...
	 * Otherwise if this Report has previously been saved, it will retrieve
	 * related ContractExcludedReports from storage. If this Report is new, it will return
		...
		if ($this->collContractExcludedReports === null) {
			if ($this->isNew()) {
			   $this->collContractExcludedReports = array();
				...
				$criteria->add(ContractExcludedReportPeer::REPORT_CODE, $this->code);
	 ...
	 */
	public function getContractExcludedReportsJoinContract($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(ReportPeer::DATABASE_NAME);
		}
		...
		if ($this->collContractExcludedReports === null) {
			if ($this->isNew()) {
				...
				$this->collContractExcludedReports = ContractExcludedReportPeer::doSelectJoinContract($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
