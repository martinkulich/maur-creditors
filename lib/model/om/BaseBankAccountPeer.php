...
/**
 * Base static class for performing query and update operations on the 'bank_account' table.
 *
 * 
 *
...
abstract class BaseBankAccountPeer {
	...
	const CLASS_DEFAULT = 'lib.model.BankAccount';
	...
	const NUMBER = 'bank_account.NUMBER';
	...
	private static $fieldNames = array (
		BasePeer::TYPE_PHPNAME => array ('Id', 'Name', 'Number', ),
		BasePeer::TYPE_STUDLYPHPNAME => array ('id', 'name', 'number', ),
		BasePeer::TYPE_COLNAME => array (self::ID, self::NAME, self::NUMBER, ),
		BasePeer::TYPE_FIELDNAME => array ('id', 'name', 'number', ),
	...
	private static $fieldKeys = array (
		BasePeer::TYPE_PHPNAME => array ('Id' => 0, 'Name' => 1, 'Number' => 2, ),
		BasePeer::TYPE_STUDLYPHPNAME => array ('id' => 0, 'name' => 1, 'number' => 2, ),
		BasePeer::TYPE_COLNAME => array (self::ID => 0, self::NAME => 1, self::NUMBER => 2, ),
		BasePeer::TYPE_FIELDNAME => array ('id' => 0, 'name' => 1, 'number' => 2, ),
		BasePeer::TYPE_NUM => array (0, 1, 2, )
	);
	...
	public static function addSelectColumns(Criteria $criteria)
	{
		...
		$criteria->addSelectColumn(BankAccountPeer::NUMBER);
	}
	...
	public static function doCount(Criteria $criteria, $distinct = false, PropelPDO $con = null)
	{
		...
		{
		  call_user_func($sf_hook, 'BaseBankAccountPeer', $criteria, $con);
		}
	...
	{
		// invalidate objects in OutgoingPaymentPeer instance pool, since one or more of them may be deleted by ON DELETE CASCADE rule.
	...
	{
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseBankAccountPeer:doInsert:pre') as $sf_hook)
    {
      if (false !== $sf_hook_retval = call_user_func($sf_hook, 'BaseBankAccountPeer', $values, $con))
      {
    ...
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseBankAccountPeer:doInsert:post') as $sf_hook)
    {
      call_user_func($sf_hook, 'BaseBankAccountPeer', $values, $con, $pk);
    }
	...
	{
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseBankAccountPeer:doUpdate:pre') as $sf_hook)
    {
      if (false !== $sf_hook_retval = call_user_func($sf_hook, 'BaseBankAccountPeer', $values, $con))
      {
        return $sf_hook_retval;
    ...
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseBankAccountPeer:doUpdate:post') as $sf_hook)
    {
      call_user_func($sf_hook, 'BaseBankAccountPeer', $values, $con, $ret);
    }
	...
	/**
	 * Method to DELETE all rows from the bank_account table.
	 *
	 * @return     int The number of affected rows (if supported by underlying database driver).
	 */
	...
	static private function getMixerPreSelectHook($method)
	{
	  ...
	  {
	    return sprintf('BaseBankAccountPeer:%s:%1$s', 'Count' == $match[1] ? 'doCount' : $match[0]);
	  }
