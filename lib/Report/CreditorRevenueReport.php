    ...
    public function getSqlPatter()
    {
        return "
            SELECT 
                ...
                sum(contract_balance(c.id, '%date_to%'::date))::integer AS current_balance,
                sum(contract_balance(c.id, '%date_to%'::date)) - creditor_received_payments(cr.id, '%date_to%'::date) as balance_change,
